...
size_t process_fdt_property(const struct fdt_header* fdt,
  ...
  return sizeof(struct fdt_property) +
         (__bswap_32(fdt_prop->len) + sizeof(uint32_t) - 1) / sizeof(uint32_t) *
             sizeof(uint32_t);
}
...
void* next_fdt_obj(const struct fdt_header* fdt, void* fdt_obj, int* depth,
  ...
  switch (tag) {
    case FDT_PROP:
      ...
      next_fdt_obj =
          fdt_obj +
          process_fdt_property(fdt, (struct fdt_property*)fdt_obj, callback);
      break;
    ...
    case FDT_BEGIN_NODE:
      ...
      if (depth) (*depth)++;
      next_fdt_obj =
          fdt_obj + process_fdt_begin(fdt, (struct fdt_node_header*)fdt_obj);
      break;
